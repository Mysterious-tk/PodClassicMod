...
class CoverFlowView(context: Context) : ScreenView, FrameLayout(context) {
    ...
    private fun setTexts(index: Int) {
        album.text = albums[index].title
        artist.text = albums[index].subtitle
    }
    ...
    private fun scaleImageView(centerX: Int, imageView: ImageView) {
        ...
        imageView.layout(
            centerX - halfImageWidth - scale,
            centerY - halfImageWidth - scale,
            centerX + halfImageWidth + scale,
            centerY + halfImageWidth + scale
        )
    }
    ...
    private val animatorListener = object : Animator.AnimatorListener {
        ...
        override fun onAnimationEnd(animation: Animator?) {
            ...
            /*
            if (slides <= 12) {
                duration = (duration * (0.75f + 0.08f * (s(abs(slides).toFloat())))).toLong()
                duration = min(duration, DEFAULT_DURATION)
            } else {
                duration = (duration * (0.75f + 0.12f * (1f - s(duration.toFloat())))).toLong()
                duration = max(duration, MIN_DURATION)
            }
